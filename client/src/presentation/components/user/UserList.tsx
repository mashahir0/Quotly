...
//   // Merge new users with previous users
//   useEffect(() => {
//     if (!data?.users) return;
...
//           (user) => !prev.some((u) => u._id === user._id)
//         );
//         return [...prev, ...newUsers]; // Avoid duplicates and add new users
//       });
//     }
//   }, [data, page]);

//   // Socket listener (optimistic update)
//   useEffect(() => {
//     const handleUserUpdate = (newUser: User ) => {
//       setAllUsers((prev) => {
//         return [newUser, ...prev]; // Add new user at the top
//       });
//     };
...
// import { useGetUsersChatQuery, useMarkMessagesAsSeenMutation } from "../../../data/api/chatApi";
// import { motion, AnimatePresence } from "framer-motion";
// import socket from "../../../utils/socket";
// import { useSelector } from "react-redux";
// import { RootState } from "../../../domain/redux/store";
...
//   useEffect(() => {
//     const handleUserUpdate = () => refetch();
//     socket.on("userListUpdate", handleUserUpdate);
  
//     return () => {
//       socket.off("userListUpdate", handleUserUpdate); // just call it, don't return anything
//     };
//   }, [refetch]);
...
//       });
//     },
//     [messagesender, onSelectUser, markSeen]
//   );
...
//         {isLoading && <p className="text-white">Loading users...</p>}
//         {isError && <p className="text-red-500">Error loading users</p>}
//         {!isLoading && debouncedSearch.length > 0 && users.length === 0 && (
//           <p className="text-gray-400 text-center">No users found</p>
//         )}
...
//                   transition={{ duration: 0.3 }}
//                   className={`flex items-center justify-between p-2 rounded-md cursor-pointer hover:bg-gray-700 ${
//                     selectedUserId === user._id ? "bg-blue-600 text-white" : "text-white"
//                   }`}
//                   onClick={() => handleSelectUser(user._id)}
//                 >
//                   <div className="flex items-center gap-2">
...
//                   {showDot && <div className="w-2.5 h-2.5 bg-blue-500 rounded-full mr-2" />}
//                 </motion.li>
//               );
//             })}
//           </AnimatePresence>
...
// export default React.memo(UserList); // Add memo to prevent unnecessary parent re-renders
